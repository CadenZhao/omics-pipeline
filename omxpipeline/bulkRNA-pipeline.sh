...
Option:
    ...
    -g    genome star index directory. It is generated by STAR --runMode genomeGenerate.
          Index will be built from fasta file if it not set
...
cat ${gtf_file} | grep -P '\tgene\t' | \
    awk 'BEGIN{FS="[\t; ]"; OFS="\t"; printf "chromosome\tstart\tend\tensembl_id\tstrand\thgnc_symbol\tgene_biotype\n"} \
              ...
              ${out_dir_quantification}/`basename ${gtf_file%"gtf"}`gene.annot.bed

# count2TPM, this rscript is only used internally in this pipeline
# position parameter: 1. GTF, 2. gene annot bed generated by the last step, 3. gene counts, 4. output directory
Rscript ${BASEDIR}/r/exon.length.per.gene.R ${gtf_file} ${out_dir_quantification}/`basename ${gtf_file%"gtf"}`gene.annot.bed \
        ${out_dir_quantification}/star.ReadsPerGene.out.formatted.merged.tsv ${out_dir_quantification}/tpm.gene.tsv && \
